...
def decoded_sequences_to_file(outfile_name: str,
    ...
    neg_data_path = _REPO_ROOT / "data" / "whitelab_data" / "hemo-negative.npz"
    pos_data_path = _REPO_ROOT / "data" / "whitelab_data" / "hemo-positive.npz"
    neg_data = np.load(neg_data_path)
    pos_data = np.load(pos_data_path)

    # Process sequences and write to files if needed
    df_negative = write_sequences_to_list(str(_REPO_ROOT / "data" / "data_for_data_viewer" / f"{outfile_name}_raw_negativ.txt"), neg_data['arr_0'], tokenizer)
    df_positive = write_sequences_to_list(str(_REPO_ROOT / "data" / "data_for_data_viewer" / f"{outfile_name}_raw_positive.txt"), pos_data['arr_0'], tokenizer)
...
def format_whitelab_sequences(df: pd.DataFrame, outfile_name: str, special_tokens: list[str], verbose: bool = False):
    ...
    result_df.to_csv(str(_REPO_ROOT / "data" / "data_for_data_viewer" / f"{outfile_name}_formatted.csv"), sep=';', index=False)
...
def produce_whitelab_csv_file(positive_data: pd.DataFrame, negative_data: pd.DataFrame, special_tokens: list[str]):
    ...
    result_df.to_csv(str(_REPO_ROOT / "data" / "train_data" / "whitelab_hemo_data.csv"), sep=';', index=False)
...
def download_and_prepare_whitelab_data(create_whitelab_csv: bool = False, verbose: bool = False):
    ...
    def download_hemolysis():
        urllib.request.urlretrieve(
            ...
            str(_REPO_ROOT / "data" / "whitelab_data" / "hemo-positive.npz"),
        )
        urllib.request.urlretrieve(
            'https://github.com/ur-whitelab/peptide-dashboard/raw/master/ml/data/hemo-negative.npz',
            str(_REPO_ROOT / "data" / "whitelab_data" / "hemo-negative.npz"),
        )
    ...
    def func2(task):
        func1(str(_REPO_ROOT / "data" / "whitelab_data" / f"{task}-positive.npz"))
        func1(str(_REPO_ROOT / "data" / "whitelab_data" / f"{task}-negative.npz"))
