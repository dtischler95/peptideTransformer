...
class PeptideCurriculumDataCollator(DataCollatorForLanguageModeling):
    ...
    The update strategy can be adjusted this is just a naive implementation. I'm not sure if different
    strategies yield better results.
    Strategy could be increasing the mlm probability if the current epoch deviates from the mean loss of the last n epochs.
    ...
    def __call__(self, examples, **kwargs):
        ...
        if self.mlm:
            ...
            # Update batch with masked input and labels
